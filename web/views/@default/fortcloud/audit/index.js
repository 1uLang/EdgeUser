Tea.context(function () {
    ...
    this.onReplay = function (item) {
        teaweb.confirm("确定要回放该会话吗？", function () {
            ...
            // this.onTestReplay("http://127.0.0.1:8002/fortcloud/audit/replay?id="+item.id)
        })
    }
    ...
    this.onPlayReplay = function () { 
        if(this.bAudioDisabled && !this.recording){
            this.onLoadReplay("http://127.0.0.1:8002/fortcloud/audit/replay?id="+this.selectReplayItemData.id)
        }else{
            if(this.recording){
