...
func (this *StatusAction) RunPost(params struct {
	...
	for _, serverId := range params.ServerIds {
		go func(serverId int64) {
			...
			// 检查是否属于自己的
			_, err := this.RPC().ServerRPC().CheckUserServer(this.UserContext(), &pb.CheckUserServerRequest{ServerId: serverId})
			if err != nil {
				this.ErrorPage(err)

				m["type"] = "serverErr"
				m["message"] = "服务器错误"
				m["todo"] = "错误信息：CheckUserServer(): " + err.Error() + "，请联系管理员修复此问题"
				return
			}

			// 检查cname
			serverDNSResp, err := this.RPC().ServerRPC().FindEnabledServerDNS(this.UserContext(), &pb.FindEnabledServerDNSRequest{ServerId: serverId})
			if err != nil {
				this.ErrorPage(err)
			...
			// 检查DNS是否已经设置
			serverNamesResp, err := this.RPC().ServerRPC().FindServerNames(this.UserContext(), &pb.FindServerNamesRequest{ServerId: serverId})
			if err != nil {
				this.ErrorPage(err)
