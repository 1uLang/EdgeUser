...
func (this *ClientsAction) RunGet(params struct {
	...
	if statIsOn {
		{
			resp, err := this.RPC().ServerClientSystemMonthlyStatRPC().FindTopServerClientSystemMonthlyStats(this.UserContext(), &pb.FindTopServerClientSystemMonthlyStatsRequest{
		...
		{
			resp, err := this.RPC().ServerClientBrowserMonthlyStatRPC().FindTopServerClientBrowserMonthlyStats(this.UserContext(), &pb.FindTopServerClientBrowserMonthlyStatsRequest{
				ServerId: params.ServerId,
				Month:    month,
				Offset:   0,
				Size:     10,
