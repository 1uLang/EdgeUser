...
func (this *AllowListAction) RunGet(params struct {
	...
	listId, err := dao.SharedIPListDAO.FindAllowIPListIdWithServerId(this.UserContext(), params.ServerId)
	if err != nil {
		this.ErrorPage(err)
		return
	}
	...
	// 创建
	if listId == 0 {
		listId, err = dao.SharedIPListDAO.CreateIPListForServerId(this.UserContext(), params.ServerId, "white")
		if err != nil {
			this.ErrorPage(err)
			return
		}
