...
func (this *ProvincesAction) RunGet(params struct {
	...
	// 当前选中的省份
	policyConfig, err := dao.SharedHTTPFirewallPolicyDAO.FindEnabledHTTPFirewallPolicyConfig(this.UserContext(), params.FirewallPolicyId)
	if err != nil {
		this.ErrorPage(err)
		return
	}
	...
	provincesResp, err := this.RPC().RegionProvinceRPC().FindAllEnabledRegionProvincesWithCountryId(this.UserContext(), &pb.FindAllEnabledRegionProvincesWithCountryIdRequest{
...
func (this *ProvincesAction) RunPost(params struct {
	...
	_, err = this.RPC().HTTPFirewallPolicyRPC().UpdateHTTPFirewallInboundConfig(this.UserContext(), &pb.UpdateHTTPFirewallInboundConfigRequest{
