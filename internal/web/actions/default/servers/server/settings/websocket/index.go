...
func (this *IndexAction) RunPost(params struct {
	...
	if websocketConfig.Id <= 0 {
		createResp, err := this.RPC().HTTPWebsocketRPC().CreateHTTPWebsocket(this.UserContext(), &pb.CreateHTTPWebsocketRequest{
		...
		_, err = this.RPC().HTTPWebsocketRPC().UpdateHTTPWebsocket(this.UserContext(), &pb.UpdateHTTPWebsocketRequest{
